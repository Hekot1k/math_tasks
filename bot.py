...
RANDOM_TASKS = {
    "💣 Сложные": [("Марианна задумала четырехзначное число. Она сказала подругам, что оно кратно30, его вторая цифра— 2, и оно меньше3500. Сколько чисел подходят под условия загадки Марианны?", "9"),
("Задумали трёхзначное число, все цифры которого различны и первая цифра которого четная. Из него вычли трехзначное число, записанное теми же цифрами в обратном порядке. Получили число495. Найдите сумму наименьшего и наибольшего чисел, удовлетворяющих таким условиям.", "1494."),
("Задумали трёхзначное число, последняя цифра которого не равна нулю. Из него вычли трёхзначное число, записанное теми же цифрами в обратном порядке. Получили число 792. Найдите все числа, обладающие таким свойством.", "901"),
("Олег написал на доске четырехзначное число, а потом стер первую и последнюю цифры. Позже Олег захотел снова записать это число, но забыл стертые цифры. Все, что он помнит— его число было кратно15 и вторая и третья цифры— 1 и5. Какое число могло быть написано на доске?Запишите все числа от самого маленького до самого большого по возврастанию(через запятую)", "3150, 6150, 9150, 1155, 4155, 7155.")],
    "🎯 Средние": [("Автомобиль едет по дороге, проезжая 20 метров за каждую секунду. Выразите скорость автомобиля в километрах в час.", "72"),
("Во время распродажи холодильник продавался со скидкой 11%. Сколько рублей составила скидка, если до скидки холодильник стоил 22000 рублей?", "2420"),
("Самолёт, находящийся в полёте, преодолевает 145 метров за каждую секунду. Выразите скорость самолёта в километрах в час.", "522"),
("Поезд идёт со скоростью 234км/ч. Сколько метров он проезжает за одну секунду?", "65"),
("Поезд идёт со скоростью 828 км/ч. Сколько метров он проезжает за одну секунду?", "230")],
    "🍀 Легкие": [("В треугольнике два угла равны 36° и 73°. Найдите его третий угол. Ответ дайте в градусах.", "71"),
("Два внешних угла треугольника при разных вершинах равны. Периметр треугольника равен 78см, а одна из сторон равна 18см. Найдите две другие стороны треугольника. Ответ запишите в виде двух чисел, идущих подряд, без лишних знаков.", "3030"),
("В прямоугольном треугольнике ABC с прямым углом C проведена высота CD. Найдите величину угла B, если DA=12, а AC=24. Ответ дайте в градусах. Запишите решение и ответ.", "30"),
("В равнобедренном треугольнике АВС с основанием АВ угол С в 2 раза меньше угла А. Найдите величину внешнего угла при вершине В. Ответ дайте в градусах. Запишите решение и ответ.", "108")]
}
...
async def random_task(message: types.Message):
    all_tasks = [task for category in RANDOM_TASKS.values() for task in category]
...
@dp.message_handler(text='📊 Сложность')
...
async def difficulty_selected(message: types.Message):
    tasks = RANDOM_TASKS[message.text]
    task = choice(tasks)
